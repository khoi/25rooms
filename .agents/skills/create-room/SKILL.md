...
Turn a city into exactly 25 new miniature scenes, with enough researched detail for each room to be drawn and animated. Use the original collection as a minimum craft benchmark and aim for greater visual invention, material depth, and storytelling while letting the new city's geography, people, materials, and daily rhythms determine its subjects.
...
- In the 25rooms repository, inspect the original rooms in `dist/artwork/drawings.js`, the collection registry, and existing city briefs and rooms. Read [the original-room benchmark](references/original-room-depth.md) and [art direction beyond the benchmark](references/art-direction.md) before designing or revising artwork. Render the originals and study their code; a prose brief or another recent city collection is not sufficient as the quality reference.
...
Design detail at three viewing scales: a memorable silhouette in the collection, several readable clusters at normal zoom, and small discoveries up close. Give every room a signature architectural shape and a genuinely elaborate central object or natural feature. Their silhouettes, construction, and relationship to the people must carry the scene before small accessories are considered.
...
- Use 60–90 meaningful details as an inventory prompt where appropriate, but never increase a numeric target to repair weak art direction. Judge the visible complexity of architecture, focal objects, materials, light, and human relationships. Count repeated stock as a group. A scene can contain many objects and still look simple.
...
Build at least two substantial assemblies in each room: for example, a fitted preparation counter and an operating machine, or a window bay and an intricate planted bed. Give each visible construction, depth, material transitions, contents, and connections to neighboring objects. Make their construction legible: structural frame, layered surfaces, internal spaces, operational parts, contact points, and distinctive contents. Develop the dominant object until its silhouette and material behavior are specific to this room. A rectangular box with a handle is a starting shape, not a completed fixture.
...
For a density revision, identify the largest visual weaknesses at thumbnail and normal size. Redraw weak architecture and focal fixtures, replacing obsolete simple geometry and moving supporting objects as needed. Repeated end-of-draw accessory functions preserve the old composition and are not a redesign. Integrate replacements into the correct drawing phase; do not accumulate successive decoration passes.

Complete one ambitious room per collection and compare before, after, and a relevant original at the same displayed width. The change must be obvious before zooming in. Resolve a weak prototype before scaling the approach to the remaining rooms. Use the art-direction rubric to explain which visual qualities improved, and keep iterating if the new room still feels less developed. Continue autonomously within the requested implementation scope.
...
Validate the new collection's count, IDs, drawing coordinates, and animation states with the project's existing checks, updating hardcoded collection assumptions where needed. Review representative transitions and loop boundaries, and follow the session's rules for browser testing. Do not report implemented rooms on the strength of the design brief alone. Passing automated checks establishes correctness, not artistic quality. Never describe a collection as matching or surpassing the originals unless the rendered comparison supports that judgment; disclose remaining visual gaps honestly.

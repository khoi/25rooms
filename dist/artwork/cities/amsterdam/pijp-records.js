...
import { timber, metal, cushion, vessel, bentTube, benchFrame } from '../materials.js';
import { cabinetFrame } from '../structure.js';
import { windowBay, panelFront, taskLight, wallCourse, hangingRail, floorShadow } from '../joinery.js';
...
const room=world('amsterdam-pijp-records','The sleeve opens after work',{floor:'paper',tone:.28,wall:'blue',wallTone:.68,height:3.65,pattern:'tiles',accent:'teal',head:28},(H,R)=>{
  ...
  hangingRail(H,R,'nw',2.0,3.3,2.75,3,(P,u,n)=>{
  ...
  benchFrame(H,R,2.72,4.17,5.95,2.36,1.18,'sun');
  ...
  metal(H,R,3.43,7.35,2.38,1.35,.13,.07,'teal');
  box(H,R,3.7,7.62,.93,.61,.21,.1,'paper',1);shape(H,R,H.tile(3.71,7.62,.27,.22,.32),'coral',.6,.5);
  const bp=H.p(4.95,7.65,.21);shape(H,R,[[bp[0]-6,bp[1]],[bp[0]+6,bp[1]],[bp[0]+5,bp[1]-6],[bp[0]-5,bp[1]-6]],'sun',.7,.6);for(let n=-4;n<5;n+=1.3)H.line(R,[[bp[0]+n,bp[1]],[bp[0]+n,bp[1]+5]],'blue',.6);
  cushion(H,R,5.09,8.05,.43,.32,.23,.06,'coral');
  oval(H,R,...H.p(3.93,8.25,.22),5,2.7,'sun',.65);box(H,R,3.84,8.15,.17,.16,.25,.09,'blue',.7);
  const be=H.p(5.45,7.65,.23);oval(H,R,...be,5.5,3.1,'teal',.7);H.line(R,[[be[0]+5,be[1]],[be[0]+12,be[1]-3]],'blue',2);

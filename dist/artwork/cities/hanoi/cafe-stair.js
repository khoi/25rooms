...
import { windowBay, panelFront, taskLight } from '../joinery.js';
...
const room = world('hanoi-cafe-stair', 'The cup above the courtyard', { wall: false, floor: 'paper', tone: .8, pattern: 'tiles', accent: 'teal', head: 95 }, (H, R) => {
  ...
  cabinetFrame(H,R,1.05,2.7,2.25,1.25,.02,1.18,2,'teal',(i,j,w,d,z,h,n)=>{
    if(n===0){for(let k=0;k<3;k++) timber(H,R,i+.12,j+.2,w-.25,.52,z+k*.22,.16,'sun');}
    else {vessel(H,R,i+.5,j+.4,z+.32,9,15,'teal');drape(H,R,i,j,w,.64,z+.75,.35,'paper');}
  });
  benchFrame(H,R,3.65,3.42,4.05,1.8,1.29,'sun');

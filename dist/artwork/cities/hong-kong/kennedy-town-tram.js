...
function body(H, R) {
  ...
  for (const i of [2.45, 5.98, 8.75]) {
    metal(H, R, i, 3.98, 0.11, 3.69, 0.18, 0.16, 'blue');
    for (let n = 0; n < 4; n++) metal(H, R, i - 0.31, 7.32, 0.7, 0.25, 0.27 + n * 0.06, 0.04, 'teal');
  }
  bentTube(H, R, [[2.65, 7.45, 0.3], [5.92, 7.45, 0.3], [8.81, 6.56, 0.37]], 2.4, 'sun');
  metal(H, R, 4.3, 4.34, 2.1, 0.7, 0.8, 0.32, 'teal');
  for (let n = 0; n < 7; n++) metal(H, R, 4.43 + n * 0.27, 4.99, 0.13, 0.04, 0.88, 0.16, 'blue');
  ...
  for (const i of [1.64, 4.1, 6.65]) {
    bentTube(H, R, [[i, 4.01, 4.4], [i, 4.74, 4.4], [i, 4.77, 4.03]], 1.6, 'sun');
    for (let n = 0; n < 4; n++) bentTube(H, R, [[1.65, 4.1 + n * 0.17, 4.39], [6.67, 4.1 + n * 0.17, 4.39]], 0.8, 'teal');
  }
  metal(H, R, 5.1, 4.2, 1.06, 0.5, 4.43, 0.3, 'coral');
  bentTube(H, R, [[5.39, 4.48, 4.75], [5.39, 4.48, 4.92], [5.77, 4.48, 4.92], [5.77, 4.48, 4.75]], 1.2, 'sun');
  for (const i of [5.25, 5.98]) metal(H, R, i, 4.72, 0.09, 0.035, 4.46, 0.25, 'sun');
  ...
  metal(H, R, 9.65, 7.32, 0.69, 0.045, 0.92, 0.38, 'blue');
  for (let n = 0; n < 5; n++) H.line(R, [H.p(9.73 + n * 0.11, 7.38, 0.98), H.p(9.73 + n * 0.11, 7.38, 1.22)], 'paper', 0.7);
  bentTube(H, R, [[10.42, 6.91, 1.72], [10.42, 6.91, 1.18], [9.58, 7.2, 0.82]], 1.2, 'coral');
  metal(H, R, 8.51, 7.25, 0.47, 0.26, 0.78, 0.08, 'blue');
  for (let n = 0; n < 3; n++) H.line(R, [H.p(8.57, 7.31 + n * 0.06, 0.88), H.p(8.93, 7.31 + n * 0.06, 0.88)], 'paper', 0.7);
...
const room = world(
  ...
  (H, R) => {
    ...
    cabinetFrame(H, R, 3.24, 9.73, 2.21, 1.16, 0.06, 0.81, 2, 'teal', (i, j, w, d, z, h, n) => {
      coiledLine(H, R, i + w / 2, j + d / 2, z + 0.1, 10, n ? 'sun' : 'coral');
    });
    metal(H, R, 3.45, 9.84, 1.01, 0.76, 0.9, 0.12, 'paper');
    bentTube(H, R, [[3.6, 10.2, 1.04], [4.15, 10.2, 1.04], [4.37, 10.35, 1.04]], 2, 'sun');
    for (const i of [3.39, 5.24]) bentTube(H, R, [[i, 9.72, 0.2], [i, 9.72, 1.34], [i, 10.1, 1.34]], 1.8, 'teal');
    body(H, R);
  },
  (H, R, t) => {
    const u = cycle(t, 20);

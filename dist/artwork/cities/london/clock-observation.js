...
import { surface, timber, metal, bentTube, benchFrame, drape, cushion } from '../materials.js';
...
const room = world('london-clock-observation', 'The shadow passes the mark', { wall: false, floor: 'paper', head: 80 }, (H, R) => {
  ...
  benchFrame(H, R, 9.92, 8.25, 1.4, 1.35, .65, 'teal');
  cushion(H, R, 10.01, 8.36, 1.19, 1.08, .67, .12, 'sun');
  bentTube(H, R, [[10.03, 8.36, .7], [10.03, 8.23, 1.49], [11.13, 8.23, 1.49], [11.13, 8.36, .7]], 3, 'teal');
  drape(H, R, 10.03, 8.6, .55, .79, .8, .61, 'coral');

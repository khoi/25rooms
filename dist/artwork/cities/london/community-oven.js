...
import { windowBay, panelFront, caster, taskLight, hangingRail, wallRack } from '../joinery.js';
...
function central(H,R){
 ...
 for(const i of [3.78,6.13]){metal(H,R,i,2.53,.1,2.5,.12,1.99,'teal');
 metal(H,R,i-.055,2.48,.21,.21,.06,.12,'blue');metal(H,R,i-.055,4.85,.21,.21,.06,.12,'blue');
 H.line(R,[H.p(i,2.62,.31),H.p(i,4.77,1.8)],'blue',1.4);
 metal(H,R,i-.05,2.5,.2,2.72,1.08,.07,'blue');metal(H,R,i-.05,2.5,.2,2.72,1.87,.07,'blue');}
 ...
 for(const j of [2.56,4.93]){H.line(R,[H.p(3.81,j,1.97),H.p(6.12,j,1.97)],'paper',1.8);for(const i of [3.79,6.11])oval(H,R,...H.p(i,j,1.12),2.4,1.8,'sun',.85);}
 for(const z of [.52,.76])metal(H,R,3.92,2.65,1.98,1.62,z,.05,'blue');
 for(let n=0;n<8;n++)H.line(R,[H.p(3.91+n*.28,2.88,2.02),H.p(3.91+n*.28,4.51,2.02)],'paper',.85);
 timber(H,R,7.71,2.77,2.8,2.33,.37,.11,'teal');
 for(let n=0;n<2;n++){box(H,R,7.88+n*1.33,3.06,1.12,1.68,.5,.39,n===1?'paper':'sun',.65);H.line(R,[H.p(8.16+n*1.33,4.76,.68),H.p(8.61+n*1.33,4.76,.68)],'blue',1.7);}
 ...
 metal(H,R,7.48,3.15,.13,1.33,.99,.12,'blue');
 drape(H,R,7.38,3.5,.31,.74,1.16,.52,'paper');
 shape(H,R,H.tile(9.33,4.17,1.12,.83,1.15),'sun',.3);
 const scraper=H.p(10.08,4.45,1.19);shape(H,R,[[scraper[0]-7,scraper[1]-6],[scraper[0]+6,scraper[1]-9],[scraper[0]+8,scraper[1]+3],[scraper[0]-6,scraper[1]+5]],'paper',1);H.line(R,[[scraper[0]-7,scraper[1]-6],[scraper[0]+6,scraper[1]-9]],'teal',3);
...
const room=world('london-community-oven','A tray for the gathering',{wall:false,pattern:'tiles',floor:'paper',accent:'teal',head:50},(H,R)=>{
 ...
 metal(H,R,1.63,.3,.14,1.54,.09,.24,'blue');metal(H,R,4.45,.3,.14,1.54,.09,.24,'blue');
 shape(H,R,H.faceJ(4.76,.49,1.18,.44,2.39),'teal',.26);
 for(let n=0;n<8;n++)H.line(R,[H.p(4.78,.66+n*.12,.73),H.p(4.78,.66+n*.12,1.13)],'blue',.9);
 metal(H,R,1.64,1.89,2.98,.14,2.18,.46,'teal');
 metal(H,R,1.69,1.91,2.88,.16,.35,.16,'blue');
 ...
 for(let n=0;n<9;n++)H.line(R,[H.p(1.76+n*.33,1.8,2.89),H.p(1.83+n*.33,1.58,2.89)],'blue',1.1);
 H.line(R,[H.p(1.83,1.98,.6),H.p(4.42,1.98,.6)],'paper',2.1);
 metal(H,R,4.67,1.5,.13,.28,2.42,.25,'coral');
 wallRack(H,R,'ne',5.16,2.66,2.44,.87,1,'sun',(P,z)=>{const a=P(.53,z+.11);shape(H,R,[[a[0]-13,a[1]],[a[0]+13,a[1]],[a[0]+11,a[1]-16],[a[0]-12,a[1]-16]],'teal',.75);oval(H,R,a[0]-5,a[1]-7,4,4,'paper',1);H.line(R,[[a[0]+5,a[1]-11],[a[0]+7,a[1]-17]],'blue',1);const b=P(1.96,z+.1);oval(H,R,...b,13,7,'paper',1);H.line(R,[[b[0]-10,b[1]],[b[0]+9,b[1]]],'coral',2);});
 hangingRail(H,R,'nw',6.12,2.51,2.67,4,(P,u,n)=>{const q=P(u,-.08);H.line(R,[q,[q[0],q[1]+11]],'blue',2);if(n===1){shape(H,R,[[q[0]-5,q[1]+10],[q[0]+5,q[1]+10],[q[0]+7,q[1]+24],[q[0]-7,q[1]+24]],'sun',.6);}else oval(H,R,q[0],q[1]+19,6+n,8+n,n===3?'coral':'paper',.85);});
 ...
 timber(H,R,.29,3.58,1.18,5.1,.35,.12,'teal');
 for(const j of [4.33,6.72]){box(H,R,.4,j,.93,1.03,.49,.36,'paper',.9);for(let n=0;n<4;n++)H.line(R,[H.p(1.35,j+.13+n*.21,.53),H.p(1.35,j+.13+n*.21,.81)],'teal',.8);}
 bentTube(H,R,[[.78,4.42,1.05],[.78,4.42,.41],[.43,4.42,.41],[.43,4.42,.12]],1.9,'blue');
 for(let n=0;n<3;n++)bowl(H,R,.85,5.74+n*.79,1.25,10,n===1?'coral':'paper');drape(H,R,.35,8.25,1.11,.61,1.24,.4,'paper');
 for(let n=0;n<4;n++){const p=wallPt(H,'ne',5.2+n*.63,1.98,-.2);shape(H,R,[[p[0]-8,p[1]-10],[p[0]+8,p[1]-10],[p[0]+8,p[1]+8],[p[0]-8,p[1]+8]],'paper',1);oval(H,R,p[0],p[1],5,3,n%2?'coral':'teal',.6);H.dot(p[0],p[1]-10,1.2,'sun');}
 ...
 for(const i of [1.83,4.11])for(const j of [8.76,10.03])caster(H,R,i,j);
 for(const i of [1.76,4.11])for(const j of [8.7,10.04])metal(H,R,i,j,.1,.11,.16,1.67,'teal');
 for(const z of [.3,.78,1.27]){metal(H,R,1.78,8.72,2.43,1.42,z,.08,'teal');metal(H,R,1.91,8.86,2.12,1.13,z+.1,.045,'blue');shape(H,R,H.tile(2.02,8.95,1.91,.92,z+.15),'paper',1);}
 bentTube(H,R,[[1.8,8.7,1.85],[1.8,10.06,1.85],[1.8,10.06,1.51]],2,'teal');
 drape(H,R,2.83,8.89,.88,1.12,1.43,.24,'paper');
 const brush=H.p(2.28,9.61,1.45);H.line(R,[[brush[0]-8,brush[1]-5],[brush[0]+3,brush[1]+1]],'sun',3);for(let n=0;n<5;n++)H.line(R,[[brush[0]+2+n*1.3,brush[1]],[brush[0]+6+n*1.3,brush[1]+4]],'blue',.7);
 timber(H,R,6.37,9.32,1.04,1.16,.36,.13,'sun');for(const i of [6.47,7.17])for(const j of [9.45,10.23])timber(H,R,i,j,.12,.13,.02,.36,'sun');
 shape(H,R,H.tile(6.48,9.46,.8,.83,.5),'teal',.3);bowl(H,R,6.9,9.91,.53,9,'paper');

...
const room = world('mexico-city-boat-canopy', 'The shade stays on the boat', { wall: false, floor: 'teal', tone: .53, head: 10 }, (H,R) => {
  ...
  metal(H,R,1.1,2.04,1.85,1.02,.5,.12,'teal');
  shape(H,R,H.tile(1.3,2.14,.7,.65,.64),'coral',.52); for(let n=0;n<7;n++) H.line(R,[H.p(1.35+n*.08,2.22,.65),H.p(1.35+n*.08,2.35,.65)],'paper',.9);
  oval(H,R,...H.p(2.45,2.5,.66),5,3,'sun'); H.dot(...H.p(2.45,2.5,.66),2,'blue');
  bentTube(H,R,[[1.3,2.9,.66],[1.6,2.84,.78],[2.22,2.9,.66]],1.5,'sun');

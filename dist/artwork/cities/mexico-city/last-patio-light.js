...
import { surface, timber, bentTube, benchFrame, cushion, drape, vessel, pendant } from '../materials.js';
import { cabinetFrame, masonry } from '../structure.js';
import { recessedFrame, panelFront } from '../joinery.js';
...
const room=world('mexico-city-last-patio-light','A place beside the kitchen',{wall:false,floor:'blue',tone:.22,head:50},(H,R)=>{
  ...
  benchFrame(H,R,.7,4.5,1.2,3.8,.52,'sun');
  ...
  benchFrame(H,R,3.55,4.12,4.1,2.52,.98,'sun');
  for(const j of [4.42,6.3])bentTube(H,R,[[3.82,j,.18],[5.6,j,.78],[7.31,j,.18]],2.6,'coral');
  drape(H,R,4.93,4.15,1.15,2.48,1.0,.28,'paper');
  const bowl=H.p(5.56,5.22,1.06);oval(H,R,...bowl,22,11,'teal',.7);surface(H,R,[[bowl[0]-20,bowl[1]-1],[bowl[0]-14,bowl[1]-13],[bowl[0]+1,bowl[1]-18],[bowl[0]+16,bowl[1]-12],[bowl[0]+21,bowl[1]-1]],'paper',1,.8);oval(H,R,bowl[0],bowl[1]-18,4,2,'coral',.7);
  cup(H,R,4.08,6.13,1.02,'teal',1.15);cup(H,R,7.07,4.58,1.02,'coral',.9);vessel(H,R,7.06,5.48,1.01,8,23,'teal',false);
  chair(H,R,3.0,3.05);
  benchFrame(H,R,9.15,5.25,1.3,3.3,.73,'teal');

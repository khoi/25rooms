...
import { metal, timber, bentTube, cushion, drape, floorLight, slattedSeat } from '../materials.js';
...
function washer(H,R,t){
  ...
  shape(H,R,ring(.99),'blue',.84,1.2);shape(H,R,ring(.88),'teal',.65,.8);shape(H,R,ring(.77),'blue',.91,.7);
  for(let n=0;n<3;n++){
...
const room=world('paris-launderette-rounds','The drum shows the room',{wall:'teal',wallTone:.2,height:3.7,floor:'paper',pattern:'tiles',head:65},(H,R)=>{
  ...
  metal(H,R,.5,.54,.88,1.17,.06,2.26,'teal');
  shape(H,R,H.faceI(.61,1.735,.62,.17,2.17),'teal',.37,.6);
  ...
  basket(H,R,5.9,8.25,.04,2.2,1.64);

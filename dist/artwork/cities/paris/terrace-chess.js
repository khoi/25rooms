...
import { windowBay, wallCourse, cornice, radiator, taskLight, floorShadow, hangingRail } from '../joinery.js';
...
const room=world('paris-terrace-chess','The move stays in the hand',{floor:'paper',tone:.45,wall:'sun',wallTone:.16,height:3.55,head: 20},(H,R)=>{
  ...
  const pc=[wallPt(H,'ne',4.6,1.45,-.12),wallPt(H,'ne',5.7,1.45,-.12),wallPt(H,'ne',5.7,2.3,-.12),wallPt(H,'ne',4.6,2.3,-.12)];shape(H,R,pc,'paper',1,.7);
  H.line(R,[pc[0],pc[2]],'teal',2);oval(H,R,...wallPt(H,'ne',5.15,1.9,-.14),8,5,'sun',.8);
  benchFrame(H,R,3.0,4.4,3.8,3.5,1.12,'sun');
  ...
  benchFrame(H,R,8.2,4.0,2.15,1.15,.89,'teal');cup(H,R,8.65,4.55,.91);bottle(H,R,...H.p(9.46,4.45,.91),'teal',.68,false);
